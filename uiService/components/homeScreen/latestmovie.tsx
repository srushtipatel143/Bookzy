...
import { setLatestMovies } from "@/store/features/latestMovie/latest-movie";
            ...
            <div key={item._id} className="movie-card p-0" onClick={() => router.push(`/explore/movie/${item._id}`)}>
              ...
              <div className="rating_card p-2">
                <p className="m-0 d-flex align-items-center fs-5 gap-1">
                  ...
                  {(item.ratingData && item.ratingData.totalRating) ? `${item.ratingData.totalRating}/10 (${item.ratingData.votes} Votes)` : 'N / A'}
                </p>
              </div>
            </div>
          ))}
